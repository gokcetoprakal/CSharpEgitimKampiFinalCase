...
namespace FinancialCrmFinalCase
{
    public partial class FrmBanks : Form
    {
        ...
        {
            #region BANKA BAKİYELERİ
            var ziraatBalance = db.Banks.Where(x => x.BankTitle == "Ziraat Bankası")
                .Select(y => y.BankBalance).FirstOrDefault() + "TL";
            lblZiraatBalance.Text = ziraatBalance.ToString();
            var yapikrediBalance = db.Banks.Where(x => x.BankTitle == "YapıKredi Bankası")
                .Select(y => y.BankBalance).FirstOrDefault();
            lblYapıKrediBalance.Text = yapikrediBalance.ToString() + "TL";
            var vakifbankBalance = db.Banks.Where(x => x.BankTitle == "VakıfBank")
                .Select(y => y.BankBalance).FirstOrDefault();
            lblVakifBankBalance.Text = vakifbankBalance.ToString() + "TL";
            #endregion

            #region BANKA HAREKETLERİ
